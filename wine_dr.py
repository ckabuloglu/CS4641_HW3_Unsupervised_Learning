...
for index, row in df.iterrows():
    col = kmeans.predict([reduced_data[index,:]])
    plt.plot(reduced_data[index, 0], reduced_data[index, 1], '.', color=C[2*col[0]], markersize=3)

plt.title('K-means clustering on the Wine dataset (ICA-reduced data)')

plt.xticks(())
plt.yticks(())
fig.savefig('figures/wine_km_ICA.png')
plt.close(fig)
...
for index, row in df.iterrows():
    plt.plot(reduced_data[index, 0], reduced_data[index, 1], '.', color=C[2*int(row[label])], markersize=3)

plt.xticks(())
plt.yticks(())

plt.title('Each label mapped on the ICA-reduced 2D graph')
fig.savefig('figures/wine_km_ICA_rankings.png')
